...
export async function handleChat(req: IncomingMessage, res: ServerResponse, deps: ConsoleApiDeps): Promise<void> {
  ...
  try {
    ...
    // Confirmation follow-up — ONE entry point shared with the orchestrator
    ...
    if (confirmOutcome.handled) {
      res.write(`data: ${JSON.stringify({ type: 'done', answer: confirmOutcome.reply, category: 'system', iterations: 0 })}\n\n`);
