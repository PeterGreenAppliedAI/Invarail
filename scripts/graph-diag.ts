...
async function main() {
  ...
  const g = db.selectGraph('invarail_memory');
  ...
  const sample = await g.query("MATCH (f:Fact) WHERE toLower(f.text) CONTAINS 'invarail' OR toLower(f.text) CONTAINS 'spark' RETURN f.senderId, left(f.text, 60) LIMIT 5");
